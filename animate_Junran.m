...
for k = 1:NT-1
    ...
    plot(1:Nn,c_ss_n(:,k)/p.c_s_n_max,'r-','LineWidth',2);
...
%     legend({'$$c_{ss,n}(x,t)$$';'$$c_{avg,n}(x,t)$$'},'interpreter','latex','FontSize',fs,0);
    ...
    plot(1:Np,c_ss_p(:,k)/p.c_s_p_max,'r-','LineWidth',2);
...
%     ylabel('$$i_{e,p}(x,t)$$','interpreter','latex')
    ...
    subplot(3,3,[7 8 9])
    hold on;
    plot(t(1:k),Volt(1:k),'b-',t(k),Volt(k),'bo','LineWidth',2,'MarkerSize',10);
    xlim([0, t(end)])
    ylim([min(Volt), max(Volt)]);
    ylabel('Voltage [V]','FontSize',fs)
    xlabel('Time [sec]','FontSize',fs);
    set(gca,'FontSize',fs);
    set(gca,'Position',[0.1 0.08 0.85 0.24])

    % % subplot(3,3,[10 11 12])
    % % hold on;
    % % plot(t(1:k),SOC(1:k),'b-',t(k),SOC(k),'bo','LineWidth',2,'MarkerSize',10);
    % % xlim([0, t(end)])
